...
public class Test {
	...
	static void showTest() {
		...
		um.setSimulationMode(0, SimulationMode.SimulateAndForwarding);
...
//		
		...
		Map<Integer, Integer> res1 = um.getAllRegisterState(1);
		for(int key : res1.keySet()){
			System.out.println("key:" + key + "  value:" + res1.get(key));
		}
		
		System.out.println("\n>>>>>>>>>>>>3号控制器总共有多少个寄存器：" + um.getTotalRegisters(1));
...
//		
		...
		um.setAllRegisterState(1, temp1);
		...
		System.out.println("\n>>>>>>>>>>>>>设置一号控制器的第二个寄存器的值");
		um.setRegisterState(0, 1, 169);
		
		System.out.println("\n>>>>>>>>>>设置一号控制器的第二个寄存器的值完成之后：");
		Map<Integer, Integer> res3 = um.getAllRegisterState(1);
