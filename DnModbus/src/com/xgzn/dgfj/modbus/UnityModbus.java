...
import com.xgzn.dgfj.modbus.controller.BaseController;
import com.xgzn.dgfj.modbus.entity.ControllerEntity;
import com.xgzn.dgfj.modbus.entity.RegisterEntity;
import com.xgzn.dgfj.modbus.util.CommonUtil;
...
public class UnityModbus implements UnityBridge {
	...
	void initControllerFromXml() {
		...
		try {
			...
			for (int i = 0; i < files.size(); i++) {
				...
				String slaveId = rootEle.getAttribute("slaveId");
				...
				ce.setSlaveId(Integer.parseInt(slaveId));
				ce.setRegisters(map);
	...
	public Map<Integer, Integer> getAllRegisterState(int controllerId) {
		...
		if(controllerId>=controllers.size())return res;
	...
	public void setRegisterState(int controllerId, int registerAddress,
			int value) {
		try {
			if(controllerId>=controllers.size())return;
